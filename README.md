...
Σήμερα λύσαμε το πρόβλημα του scaling. Η εφαρμογή δούλευε άψογα για 1000 ραντεβού. Αλλά μόλις τα ανέβασα σε 300000, έκανε 3 λεπτά να ανανεώσει το gui κάθε φορά που γινόταν προσθήκη η επεξεργασία σε οτιδήποτε στην βάση δεδομένων.

Βασικά προβλήματα
- Η σύγκριση του datetime είναι εξαιρετικά αργή. Αυτό σημαίνει ότι αν θέλω να προσθέσω ένα ραντεβού στην λίστα (που δεν είναι append operation επειδή μπορεί να μπει στο ενδιάμεσο), ενώ ο χρόνος είναι O(n), (εως index η αναζήτηση) και (lenth-index) το insertion, στην πράξη ήταν πολύ χειρότερο. Μου έκανε εντύπωση όταν συνειδητοποίησα ότι ήταν τόσο κακό bottleneck, επειδή το update και το delete δεν είχαν τέτοιο πρόβλημα.

Στρατηγική βελτιστοποίησης
1. Στην πράξη, η πιο σημαντική πληροφορία είναι τα "ραντεβού σε χ χρονική περίοδο". Το πρόγραμμα δείχνει ένα panel με 7 μέρες. Οπότε πρέπει να περιοριστούν οι υπολογισμοί στις σχετικές περιόδους και όχι σε όλη την λίστα.
2. Τα ραντεβού μπαίνουν σε ένα dictionary με key integer που είναι το hash της ημερομηνίας με βάση την περίοδο 2 ωρών και το value μια λίστα με τα ραντεβού. Αυτό θα μειώσει δραματικά τους χρόνους το insert και delete, που αντί για n=πλήθος όλων τον εγγραφών, θα είναι m=πλήθος των ραντεβού στο δύωρο
3. Το hashing θα είναι και μερικό sort παράλληλα. Η κάθε λίστα θα πρέπει να γίνει ξανά sort, αλλά θα το κάνει το κάθε component μόνο όταν του ζητηθεί. Επειδή μιλάμε για μέσο όρο 6 ραντεβού ανά λίστα, ο χρόνος θα είναι αμελητέος.
4. Θα υλοποιηθεί δυαδική αναζήτηση με βάση το hash της ημερομηνίας, με χρόνο O(log(n/k)),  όπου k=πλήθος χρονικών περιόδων
5. Το αποτέλεσμα θα είναι ότι ο πιο ακριβός υπολογισμός θα είναι O(n) μια φορά στην αρχή, και μετά όλοι οι υπόλοιποι θα είναι ή O(logk) ή O(mlogm) με πολύ μικρό m

Για την αποθήκευση στην βάση δεδομένων, εάν υπάρχει bottleneck, θα μεταφερθεί σε διαφορετικό thread.

Ίδια στρατηγική μπορεί να υλοποιηθει και για τους πελάτες, με hash τα πρώτα γράμματα από κάθε όρισμα. Λόγω περιορισμού του faker θα είναι λίγο δύσκολο να ελεχτεί η απόδοση, οπότε θα γίνει σε δεύτερο χρόνο.
